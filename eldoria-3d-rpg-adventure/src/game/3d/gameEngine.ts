...
import { createEnemyMesh, createPlayerMesh, PlayerMeshObjects } from './models';
...
export class GameEngine {
  ...
  public setPlayerLevel(level: number) {
    ...
    // Level 5-9: Tier 2 (Adept - Mantles, Runes, Spikes)
    // Level 10-14: Tier 3 (Master - Golden Armor, Halo, Floating Orbs)
    // Level 15+: Tier 4 (Archon - Radiant / Void / Celestial Wings)
    const newTier = level >= 15 ? 4 : level >= 10 ? 3 : level >= 5 ? 2 : 1;
    if (newTier !== this.playerTier) {
      this.playerTier = newTier;
      this.rebuildPlayerMesh();
      const tierTitles: Record<number, string> = {
        2: 'EVOLUSI TINGKAT 2 (Level 5)! Zirah & Senjata Diperkuat!',
        3: 'EVOLUSI TINGKAT 3 (Level 10)! Mahkota & Zirah Emas Bercahaya!',
        4: 'EVOLUSI PAMUNGKAS (Level 15)! Sayap Astral & Kekuatan Dewata!',
      };
      if (tierTitles[newTier]) {
        this.callbacks.onAddFloatingText(tierTitles[newTier], '#facc15', 'crit');
        this.spawnShockwaveRing(this.playerPos, 0xfacc15, 5.5, 0.9);
        this.spawnSparks(this.playerPos.clone().add(new THREE.Vector3(0, 1.5, 0)), 0xfacc15, 25, 5.5);
  ...
  private spawnCrescentSlash(
    ...
    const geom = new THREE.RingGeometry(radius * 0.7, radius, 24, 1, -arcAngle / 2, arcAngle);
    ...
    slash.rotation.z = tiltZ;
    slash.rotation.x = 0.2;

    let life = 0.22;
    const maxLife = 0.22;
    this.addVfx(slash, (delta) => {
      life -= delta;
      if (life <= 0) {
      ...
      slash.position.addScaledVector(direction, delta * 2.5);
      return true;
    });
  }
  ...
  // 1. Warrior Basic Slash VFX
  private spawnWarriorSlashVfx() {
    const fwd = new THREE.Vector3(0, 0, 1).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.playerRotationY);
    const spawnPos = this.playerPos.clone().add(fwd.clone().multiplyScalar(1.2)).add(new THREE.Vector3(0, 1.2, 0));
    this.spawnCrescentSlash(spawnPos, fwd, -0.4, 0xfbbf24, 1.4, Math.PI * 0.85);
    this.spawnSparks(spawnPos, 0xf59e0b, 6, 3.5);
  }

  // 2. Warrior Whirlwind VFX
  private spawnWarriorWhirlwindVfx() {
    const group = new THREE.Group();
    group.position.copy(this.playerPos);

    // Dual whirlwind rings
    const ringGeom = new THREE.TorusGeometry(2.0, 0.12, 6, 24);
    ...
    const ringUpper = new THREE.Mesh(ringGeom, ringMat);
    ringUpper.rotation.x = Math.PI / 2;
    ringUpper.position.y = 1.4;

    const ringLower = new THREE.Mesh(ringGeom, ringMat.clone());
    ringLower.rotation.x = Math.PI / 2;
    ringLower.position.y = 0.7;
    ...
    this.addVfx(group, (delta) => {
      ...
      group.position.copy(this.playerPos);
      ringUpper.rotation.z += delta * 18;
      ringLower.rotation.z -= delta * 18;
      const progress = 1 - life / 0.55;
      const scale = 1.0 + progress * 1.2;
      group.scale.set(scale, 1, scale);
      ringMat.opacity = (life / 0.55) * 0.9;
      (ringLower.material as THREE.MeshBasicMaterial).opacity = (life / 0.55) * 0.9;
      return true;
    });

    this.spawnShockwaveRing(this.playerPos, 0xfbbf24, 4.8, 0.55);
  }

  // 3. Warrior Holy Shield Charge VFX
  private spawnWarriorShieldChargeVfx() {
    const fwd = new THREE.Vector3(0, 0, 1).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.playerRotationY);
    const shieldGroup = new THREE.Group();

    const geom = new THREE.BoxGeometry(1.6, 2.0, 0.08);
    const mat = new THREE.MeshBasicMaterial({
      color: 0xfbbf24,
      transparent: true,
      opacity: 0.75,
    ...
    // Cross crest
    const crossGeom = new THREE.BoxGeometry(0.4, 1.4, 0.1);
    ...
    cross1.position.z = 0.04;
    cross2.position.z = 0.04;
    shieldGroup.add(cross1);
    shieldGroup.add(cross2);
    ...
    this.addVfx(shieldGroup, (delta) => {
      ...
      shieldGroup.position.copy(this.playerPos).add(fwd.clone().multiplyScalar(1.2)).add(new THREE.Vector3(0, 1.3, 0));
      mat.opacity = (life / 0.35) * 0.75;
      return true;
    });
  }

  // 4. Warrior Iron Fortress VFX
  private spawnWarriorIronFortressVfx() {
    const domeGeom = new THREE.SphereGeometry(3.2, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.5);
    ...
    dome.position.copy(this.playerPos);

    // Holy ground rune
    const runeGeom = new THREE.RingGeometry(1.2, 3.2, 32);
    ...
    rune.rotation.x = -Math.PI / 2;
    rune.position.copy(this.playerPos);
    ...
    this.addVfx(group, (delta) => {
      ...
      group.position.copy(this.playerPos);
      rune.rotation.z += delta * 1.5;
      const opacity = Math.min(1, life / 0.5);
      domeMat.opacity = opacity * 0.45;
      runeMat.opacity = opacity * 0.85;
      return true;
    });

    this.spawnSparks(this.playerPos.clone().add(new THREE.Vector3(0, 1, 0)), 0xfef08a, 12, 3);
  }

  // 5. Mage Arcane Bolt (Ranged Projectile)
  private spawnMageArcaneBolt() {
    ...
    // Core glowing sphere
    const coreGeom = new THREE.SphereGeometry(0.24, 12, 12);
    const coreMat = new THREE.MeshBasicMaterial({ color: 0x38bdf8 });
    const core = new THREE.Mesh(coreGeom, coreMat);
    group.add(core);

    // Outer aura
    const auraGeom = new THREE.SphereGeometry(0.38, 10, 10);
  ...
  // 6. Mage Supernova Astral VFX
  private spawnMageNovaVfx() {
    this.spawnShockwaveRing(this.playerPos, 0xc084fc, 5.8, 0.45);
    this.spawnShockwaveRing(this.playerPos, 0x38bdf8, 4.2, 0.35);
    this.spawnSparks(this.playerPos.clone().add(new THREE.Vector3(0, 1.2, 0)), 0xa855f7, 16, 6);
    this.spawnSparks(this.playerPos.clone().add(new THREE.Vector3(0, 1.2, 0)), 0x38bdf8, 14, 5);
  }

  // 7. Mage Arcane Blink VFX
  private spawnMageBlinkVfx(startPos: THREE.Vector3, endPos: THREE.Vector3) {
    // Start burst
    this.spawnShockwaveRing(startPos, 0xa855f7, 2.5, 0.3);
    this.spawnSparks(startPos.clone().add(new THREE.Vector3(0, 1.2, 0)), 0xa855f7, 10, 4);

    // End arrival burst
    this.spawnShockwaveRing(endPos, 0x38bdf8, 3.2, 0.35);
    this.spawnSparks(endPos.clone().add(new THREE.Vector3(0, 1.2, 0)), 0x38bdf8, 14, 5);
  }

  // 8. Mage Celestial Heal Light Pillar
  private spawnMageCelestialHealVfx() {
    const beamGeom = new THREE.CylinderGeometry(1.6, 1.8, 12, 16, 1, true);
    ...
    beam.position.copy(this.playerPos);
    beam.position.y += 6;
    ...
    this.addVfx(beam, (delta) => {
      ...
      beam.position.x = this.playerPos.x;
      beam.position.z = this.playerPos.z;
      beam.rotation.y += delta * 3;
      beamMat.opacity = (life / 1.0) * 0.65;
      return true;
    });

    this.spawnShockwaveRing(this.playerPos, 0x67e8f9, 3.5, 0.7);
    this.spawnSparks(this.playerPos.clone().add(new THREE.Vector3(0, 1.5, 0)), 0x67e8f9, 16, 3);
  }

  // 9. Rogue X-Slash VFX (Dual crossing emerald & shadow blades)
  private spawnRogueXSlashVfx() {
    const fwd = new THREE.Vector3(0, 0, 1).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.playerRotationY);
    const spawnPos = this.playerPos.clone().add(fwd.clone().multiplyScalar(1.0)).add(new THREE.Vector3(0, 1.2, 0));

    // Slash 1: tilted positive 40 degrees
    this.spawnCrescentSlash(spawnPos, fwd, 0.65, 0x10b981, 1.2, Math.PI * 0.7);
    // Slash 2: tilted negative 40 degrees
    this.spawnCrescentSlash(spawnPos, fwd, -0.65, 0x059669, 1.2, Math.PI * 0.7);

    this.spawnSparks(spawnPos, 0x34d399, 8, 3.5);
    this.spawnSparks(spawnPos, 0x1e1b4b, 6, 2.5);
  }

  // 10. Rogue Shadow Flurry VFX (Rapid multi-strike shadow beams)
  private spawnRogueShadowFlurryVfx() {
    ...
    for (let i = 0; i < 5; i++) {
      setTimeout(() => {
        ...
        const offsetFwd = fwd.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), (Math.random() - 0.5) * 0.5);
        const spawnPos = this.playerPos.clone().add(offsetFwd.clone().multiplyScalar(1.1 + Math.random() * 0.5)).add(new THREE.Vector3(0, 1.0 + Math.random() * 0.6, 0));
        this.spawnCrescentSlash(spawnPos, offsetFwd, (Math.random() - 0.5) * 1.5, i % 2 === 0 ? 0x10b981 : 0x6366f1, 1.1, Math.PI * 0.65);
        this.spawnSparks(spawnPos, 0x10b981, 4, 3);
      }, i * 65);
    }
  }

  // 11. Rogue Shadow Step VFX (Shadow decoy silhouettes)
  private spawnRogueShadowStepVfx(startPos: THREE.Vector3, fwd: THREE.Vector3) {
    for (let i = 1; i <= 3; i++) {
      const decoyPos = startPos.clone().add(fwd.clone().multiplyScalar(i * 3.5));
      const geom = new THREE.BoxGeometry(0.7, 1.7, 0.4);
      const mat = new THREE.MeshBasicMaterial({
        color: 0x064e3b,
        transparent: true,
        opacity: 0.7,
      ...
      const decoy = new THREE.Mesh(geom, mat);
      decoy.position.copy(decoyPos);
      decoy.position.y += 0.85;
      decoy.rotation.y = this.playerRotationY;

      let life = 0.35;
      this.addVfx(decoy, (delta) => {
        life -= delta;
        if (life <= 0) {
          this.scene.remove(decoy);
          return false;
        }
        mat.opacity = (life / 0.35) * 0.7;
        return true;
      });
  ...
  // 12. Rogue Smoke Cloak VFX (Thick ninja smoke screen)
  private spawnRogueSmokeCloakVfx() {
    const group = new THREE.Group();
    group.position.copy(this.playerPos);

    for (let i = 0; i < 12; i++) {
      const geom = new THREE.DodecahedronGeometry(0.6 + Math.random() * 0.4, 1);
      const mat = new THREE.MeshBasicMaterial({
        color: i % 2 === 0 ? 0x064e3b : 0x18181b,
        transparent: true,
        opacity: 0.7,
        depthWrite: false,
      });
    ...
    this.addVfx(group, (delta) => {
      ...
      group.children.forEach((child) => {
        ...
        child.scale.multiplyScalar(1 + delta * 0.8);
        ((child as THREE.Mesh).material as THREE.MeshBasicMaterial).opacity = (life / 1.4) * 0.7;
      });
      return true;
    });

    this.spawnShockwaveRing(this.playerPos, 0x059669, 4.0, 0.6);
  }

  // Helper to execute melee hit detection with customizable cones
  ...
  public performBasicAttack() {
    ...
    if (this.charClass === 'warrior') {
      ...
      this.executeMeleeAttack({
        ...
        coneAngle: 0.3,
        dmgMultiplier: 1.35,
        critBonus: 0.35, // High critical strike for rogue!
        knockback: 0.25,
      });
    }
  }
    ...
    if (skillSlot === 1) {
      ...
      this.callbacks.onUpdateStats((stats) => {
        ...
        if (this.charClass === 'warrior') {
          ...
          this.callbacks.onAddFloatingText('Pusaran Badai Baja!', '#fbbf24', 'crit');
        } else if (this.charClass === 'mage') {
          // Mage: Supernova Astral
          ...
          this.callbacks.onAddFloatingText('Supernova Astral!', '#c084fc', 'crit');
        } else if (this.charClass === 'rogue') {
          // Rogue: Tarian 1000 Belati (Shadow Flurry)
          ...
          this.enemies.forEach((enemy) => {
            if (enemy.data.isDead) return;
            const toEnemy = new THREE.Vector3().subVectors(enemy.mesh.position, this.playerPos);
            if (toEnemy.length() <= 3.8 && forward.dot(toEnemy.normalize()) > 0.2) {
              this.damageEnemy(enemy, skillDmg, true, 0.3);
            }
          });
          this.callbacks.onAddFloatingText('Tarian 1000 Belati!', '#34d399', 'crit');
        }
      ...
      this.callbacks.onUpdateStats((stats) => {
        ...
        if (this.charClass === 'warrior') {
          ...
          // Knockback & damage enemies in path
          setTimeout(() => {
            if (!this.isRunning) return;
            this.enemies.forEach((enemy) => {
              if (enemy.data.isDead) return;
          ...
          // Mage: Langkah Dimensi (Instant Blink Teleport)
          soundManager.playMageBlink();
          const startPos = this.playerPos.clone();
          const targetPos = this.playerPos.clone().add(forward.clone().multiplyScalar(11));

          // Bound constraints
          targetPos.x = Math.max(-68, Math.min(68, targetPos.x));
          targetPos.z = Math.max(-70, Math.min(52, targetPos.z));
          ...
          // Rogue: Langkah Siluman Bayangan
          soundManager.playDash();
      ...
      this.callbacks.onUpdateStats((stats) => {
        ...
        if (this.charClass === 'warrior') {
          // Warrior: Benteng Besi Suci (Heal & Iron Shield Dome)
          soundManager.playLevelUp();
          soundManager.playWarriorShieldSlam();
          ...
          // Mage: Hujan Cahaya Astral (Pillar of Divine Celestial Light)
          ...
          const newHp = Math.min(stats.maxHp, stats.hp + healAmt);
          const newMp = Math.min(stats.maxMp, stats.mp - 25 + 30); // Restores +30 MP!

          this.callbacks.onAddFloatingText(`+${healAmt} HP & +30 Mana Astral`, '#38bdf8', 'heal');
          return { ...stats, mp: newMp, hp: newHp };
        } else if (this.charClass === 'rogue') {
          // Rogue: Kabut Asap & Eliksir Siluman (Smoke Bomb & Agility Boost)
          soundManager.playRogueSmoke();
          soundManager.playLevelUp();
          ...
          const newHp = Math.min(stats.maxHp, stats.hp + healAmt);
          this.speedBuffTimer = 8.0; // 8 seconds agility boost!
  ...
  private updatePlayer(delta: number) {
    ...
    // 4b. Cliff climbing physics (prevents instant teleporting up sheer cliff walls)
    ...
    // If on the ground and moving into a steep cliff wall (> 0.5m step on short horizontal displacement)
    if (this.isGrounded && heightDiff > 0.55 && (horizDist < 0.001 || heightDiff / Math.max(0.01, horizDist) > 1.25)) {
      // Test sliding horizontally along X only
      ...
      // Test sliding horizontally along Z only
      ...
      if (hDiffX <= 0.45 && Math.abs(this.playerVel.x) > 0.1) {
        nextX = resXOnly.x;
        nextZ = oldZ;
      } else if (hDiffZ <= 0.45 && Math.abs(this.playerVel.z) > 0.1) {
        nextX = oldX;
        nextZ = resZOnly.z;
      } else {
        // Blocked by sheer cliff wall
        nextX = oldX;
        nextZ = oldZ;
        this.playerVel.x = 0;
        this.playerVel.z = 0;
    ...
    // Island boundary constraints (North Volcanic, South Ocean, East Forest, West Ruins)
    if (nextX < -68) nextX = -68;
    if (nextX > 68) nextX = 68;
    if (nextZ < -70) nextZ = -70;
    if (nextZ > 52) nextZ = 52; // Ocean swimming limit
    ...
    const groundY = this.world.getTerrainHeight(this.playerPos.x, this.playerPos.z);
    if (this.playerPos.z > 22 && groundY < -0.2) {
      ...
      if (this.playerPos.y <= groundY) {
        // Smoothly step up instead of instant teleport jump
        if (this.isGrounded && groundY > this.playerPos.y) {
          const stepSpeed = 12.0;
          this.playerPos.y = Math.min(groundY, this.playerPos.y + stepSpeed * delta);
          if (Math.abs(this.playerPos.y - groundY) < 0.15) {
            this.playerPos.y = groundY;
          }
        } else {
          this.playerPos.y = groundY;
  ...
  private updateNearbyPrompts() {
    ...
    if (this.world.npcs && this.world.npcs.length > 0) {
      for (const npc of this.world.npcs) {
        const dist = this.playerPos.distanceTo(new THREE.Vector3(...npc.position));
        if (dist < 3.8) {
          this.callbacks.onNearbyPrompt(
            `Tekan [F] / Sentuh untuk Bicara dengan ${npc.nameId} (${npc.role})`,
            () => {
              this.callbacks.onInteractNPC(npc.dialogueId || npc.id);
            }
          );
  ...
  private updateEnemies(delta: number) {
    ...
    this.enemies.forEach((enemy) => {
      ...
      // If player in detection range, AI chases player
    ...
    for (let i = this.activeProjectiles.length - 1; i >= 0; i--) {
      ...
      for (const enemy of this.enemies) {
        ...
        if (proj.object.position.distanceTo(enemy.mesh.position) <= 1.6) {
          ...
          this.spawnSparks(proj.object.position, 0x38bdf8, 12, 4);
          this.spawnShockwaveRing(proj.object.position, 0xc084fc, 1.8, 0.25);
          hit = true;
          break;
        }
      }
