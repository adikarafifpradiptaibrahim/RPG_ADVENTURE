...
export const WorldMapModal: React.FC<WorldMapModalProps> = ({
  ...
  // World bounds: X from -70 to +70 (width 140), Z from -75 to +55 (height 130)
  const minX = -70;
  const maxX = 70;
  const minZ = -75;
  const maxZ = 55;
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="relative flex-1 bg-stone-950/90 overflow-hidden flex items-center justify-center p-4">
            ...
            <div className="relative w-full h-full max-w-[620px] max-h-[580px] rounded-xl overflow-hidden border border-stone-700/60 shadow-inner bg-[#1e232a]">
              ...
              {/* 1. Volcanic Crater Zone (North) */}
              <div
                onClick={() => setSelectedRegion(REGIONS[4])}
                className="absolute top-0 left-0 right-0 h-[32%] bg-gradient-to-b from-[#211210] via-[#2d1815] to-[#1e232a] border-b border-red-900/40 cursor-pointer hover:brightness-110 transition-all"
              >
                <div className="absolute top-3 left-4 text-xs font-bold text-red-400/90 flex items-center gap-1.5">
                  <span>🌋</span> {REGIONS[4].nameId} (Lv. {REGIONS[4].recommendedLevel})
                </div>
                {/* Lava fissure illustration */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full border border-red-500/40 bg-red-950/40 flex items-center justify-center">
                  <div className="w-12 h-12 rounded-full bg-red-600/30 blur-sm animate-pulse" />
                  <span className="text-base">💀</span>
                </div>
              </div>

              {/* 2. Ancient Ruins (West) */}
              <div
                onClick={() => setSelectedRegion(REGIONS[2])}
                className="absolute top-[32%] left-0 w-[42%] bottom-[28%] bg-gradient-to-r from-[#263238]/90 to-transparent border-r border-cyan-900/40 cursor-pointer hover:brightness-110 transition-all"
              >
                <div className="absolute top-3 left-3 text-xs font-bold text-cyan-300/90 flex items-center gap-1.5">
                  <span>🏛️</span> {REGIONS[2].nameId} (Lv. {REGIONS[2].recommendedLevel})
                </div>
                {/* Ruins icon stamps */}
                <div className="absolute top-1/2 left-1/3 text-stone-400/70 text-sm flex gap-1">
                  <span>🏛️</span>
                  <span>⚔️</span>
                </div>
              </div>

              {/* 3. Mystic Forest (East) */}
              <div
                onClick={() => setSelectedRegion(REGIONS[3])}
                className="absolute top-[32%] right-0 w-[42%] bottom-[28%] bg-gradient-to-l from-[#1b3022]/90 to-transparent border-l border-emerald-900/40 cursor-pointer hover:brightness-110 transition-all"
              >
                <div className="absolute top-3 right-3 text-xs font-bold text-emerald-300/90 flex items-center gap-1.5 text-right">
                  <span>🌲</span> {REGIONS[3].nameId} (Lv. {REGIONS[3].recommendedLevel})
                </div>
                {/* Forest trees stamps */}
                <div className="absolute top-1/2 right-1/3 text-emerald-500/70 text-sm flex gap-1">
                  <span>🌲</span>
                  <span>🔮</span>
                </div>
              </div>

              {/* 4. Sanctuary Village (Center) */}
              <div
                onClick={() => setSelectedRegion(REGIONS[0])}
                className="absolute top-[34%] left-[28%] right-[28%] bottom-[30%] rounded-2xl bg-amber-950/30 border border-amber-600/30 cursor-pointer hover:brightness-110 transition-all flex flex-col items-center justify-center"
              >
                <span className="text-xl">⛲</span>
                <span className="text-[11px] font-bold text-amber-300 mt-1">
                  Desa Sanctuary
                </span>
                <span className="text-[9px] text-amber-200/70">Pusat Desa & Air Mancur</span>
              </div>

              {/* 5. Azure Coast & Vast Ocean (South) */}
              <div
                onClick={() => setSelectedRegion(REGIONS[1])}
                className="absolute bottom-0 left-0 right-0 h-[28%] bg-gradient-to-t from-[#023e8a] via-[#0077b6] to-[#0096c7]/60 border-t border-cyan-400/30 cursor-pointer hover:brightness-110 transition-all"
              >
                {/* Sandy Beach Ribbon */}
                <div className="absolute top-0 left-0 right-0 h-4 bg-[#fde047]/30 border-b border-yellow-500/30" />
                <div className="absolute bottom-3 left-4 text-xs font-bold text-cyan-200 flex items-center gap-1.5">
                  <span>🌊</span> {REGIONS[1].nameId} (Lv. {REGIONS[1].recommendedLevel})
                </div>
                {/* Pier and Boat stamp */}
                <div className="absolute top-3 left-1/2 -translate-x-1/2 text-cyan-100/90 text-xs flex items-center gap-2">
                  <span>⚓ Dermaga</span>
                  <span>⛵ Perahu</span>
                </div>
              </div>
