    ...
    >
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border-2 border-amber-500/40 bg-gradient-to-b from-stone-900 via-stone-950 to-black p-6 shadow-2xl shadow-black/90">
        ...
        {isSingle && (
          ...
          {/* Audio Quick Control */}
          <div className="relative flex items-center justify-between rounded-xl border border-stone-800 bg-stone-950/70 px-4 py-2.5 text-xs">
          ...
          {/* Touch Controls Toggle (Analog/D-Pad) */}
          {onToggleTouchControls && (
            <div className="relative flex items-center justify-between rounded-xl border border-stone-800 bg-stone-950/70 px-4 py-2.5 text-xs">
              <span className="text-stone-300 flex items-center gap-2">
                <Gamepad2 className="w-4 h-4 text-amber-400" />
                <span>Kontroler Layar (Analog/D-Pad)</span>
              </span>
              <button
              ...
              >
                {showTouchControls ? 'Tampil (ON)' : 'Sembunyi (OFF)'}
              </button>
            </div>
          )}
          ...
          {onOpenGraphicsSettings && (
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl border border-cyan-500/30 bg-stone-900/80 hover:bg-cyan-950/40 hover:border-cyan-500/60 px-4 py-2.5 text-xs font-semibold text-cyan-200 transition active:scale-98"
            >
              <span className="flex items-center gap-2">
                <span className="text-base">⚙️</span>
                <span>Pengaturan Grafis & Performa (PC Rendah)</span>
              </span>
              <span className="rounded bg-cyan-950/80 px-2 py-0.5 text-[10px] text-cyan-300 border border-cyan-800/60">
                Ubah Grafis
              </span>
            </button>
          )}
