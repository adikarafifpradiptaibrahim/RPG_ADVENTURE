...
export class GameEngine {
  ...
  // 360-degree blade spin VFX (full circular cutting disc & sparks)
  private spawn360BladeSpinVfx(color: number, radius = 3.6) {
    const centerPos = this.playerPos.clone().add(new THREE.Vector3(0, 1.1, 0));

    const geom = new THREE.RingGeometry(radius * 0.65, radius, 36);
    const mat = new THREE.MeshBasicMaterial({
      color,
      side: THREE.DoubleSide,
      transparent: true,
    ...
    this.addVfx(ring, (delta) => {
      ...
      const progress = 1 - life / maxLife;
      const scale = 0.65 + progress * 0.9;
      ring.scale.set(scale, scale, scale);
      ring.rotation.z += delta * 14;
      mat.opacity = (life / maxLife) * 0.95;
      return true;
    });

    this.spawnShockwaveRing(this.playerPos, color, radius * 1.15, 0.35);
    this.spawnSparks(centerPos, color, 14, 5);
  }

  // Hunter Arrow Projectile Spawner
  ...
  private updatePlayer(delta: number) {
    ...
    // Normal hill slopes (slope <= 2.2, up to ~65 degrees) or moderate elevation changes (<= 0.75m):
    // The player walks up naturally and smoothly.
    if (heightDiff > 0.75 && slope > 2.2) {
      // Sheer cliff wall!
      // If player is airborne/jumping and can mantle onto the ledge:
      const canMantle = !this.isGrounded && (this.playerPos.y + 1.1 >= targetGroundY);
      ...
      if (!canMantle) {
        ...
        if (hDiffX <= 0.65 && Math.abs(this.playerVel.x) > 0.05) {
          nextX = resXOnly.x;
          nextZ = oldZ;
        } else if (hDiffZ <= 0.65 && Math.abs(this.playerVel.z) > 0.05) {
          nextX = oldX;
          nextZ = resZOnly.z;
        } else {
    ...
    } else if (heightDiff > 0.1 && slope > 1.2) {
      // Climbing a steep hill: slight realistic incline resistance
      this.playerVel.x *= 0.95;
      this.playerVel.z *= 0.95;
    }

    // Expanded World boundary constraints (Shadowfen West, Val-Kragor East, Frostpeak North, Coral Cove South)
    if (nextX < -142) nextX = -142;
    if (nextX > 146) nextX = 146;
    if (nextZ < -125) nextZ = -125;
    if (nextZ > 78) nextZ = 78; // Ocean & Coral Cove boundary
      ...
      for (const enemy of this.enemies) {
        ...
        if (proj.object.position.distanceTo(enemy.mesh.position) <= 1.6) {
          if (proj.vfxType === 'hunter_arrow' || proj.vfxType === 'hunter_arrow_rain') {
            ...
            this.spawnSparks(proj.object.position, 0x10b981, 10, 4);
            this.spawnShockwaveRing(proj.object.position, 0x34d399, 1.5, 0.2);
          } else {
