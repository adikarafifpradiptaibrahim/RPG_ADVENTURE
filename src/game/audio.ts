...
class SoundManager {
  ...
  public playBowShoot() {
    ...
    const now = this.ctx.currentTime;
    // 1. String release twang
    const twang = this.ctx.createOscillator();
    const twangGain = this.ctx.createGain();
    twang.type = 'triangle';
    twang.frequency.setValueAtTime(520, now);
    twang.frequency.exponentialRampToValueAtTime(140, now + 0.12);

    twangGain.gain.setValueAtTime(0.35, now);
    twangGain.gain.exponentialRampToValueAtTime(0.001, now + 0.12);
