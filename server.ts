...
setInterval(() => {
  ...
  for (const [roomId, room] of Object.entries(rooms)) {
    ...
    for (const monster of Object.values(room.monsters)) {
      if (monster.isDead) {
        if (monster.respawnTimer === undefined || monster.respawnTimer <= 0) {
          monster.respawnTimer = monster.type === 'boss' ? 180 : 30;
        } else {
          ...
          if (monster.respawnTimer <= 0) {
            ...
            io.to(roomId).emit('chat_broadcast', {
              id: `sys_respawn_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
              sender: 'Sistem',
              text: monster.type === 'boss'
                ? `🔥 [BOSS RESPAWN] ${monster.name} telah bangkit kembali di Kawah Gunung Berapi!`
                : `✨ [RESPAWN] ${monster.name} telah muncul kembali di alam liar.`,
              isSystem: true,
              timestamp: Date.now(),
            });
          }
      ...
      const distFromSpawn = Math.hypot(monster.x - monster.spawnX, monster.z - monster.spawnZ);
      const leashRadius = monster.type === 'boss' ? 36 : 22;

      if (closestPlayer && closestDist <= monster.detectRange && distFromSpawn <= leashRadius) {
        ...
        // Return to spawn point if drifted away from territory and slowly regenerate
        const sdx = monster.spawnX - monster.x;
        const sdz = monster.spawnZ - monster.z;
        const sdist = Math.hypot(sdx, sdz);
        if (sdist > 1.0) {
          const returnStep = monster.speed * 0.65 * dt;
          monster.x += (sdx / sdist) * returnStep;
          monster.z += (sdz / sdist) * returnStep;
          monster.rotationY = Math.atan2(sdx, sdz);

          // Regenerate health slowly while walking back to territory
          if (monster.hp < monster.maxHp) {
            monster.hp = Math.min(monster.maxHp, monster.hp + monster.maxHp * 0.15 * dt);
          }
        } else {
          // Arrived at territory spawn point, full health restored
          monster.hp = monster.maxHp;
        }
      }
    }
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('player_attack', (data: {
    ...
    socket.to(currentRoom).emit('remote_player_attack', {
      ...
      charClass: data.charClass || rooms[currentRoom]?.players[playerId]?.charClass,
      hunterWeaponMode: data.hunterWeaponMode,
      x: data.x,
      y: data.y,
      z: data.z,
  ...
  socket.on('monster_hit', (data: {
    ...
    if (monster) {
      ...
      if (monster.hp <= 0) {
        monster.isDead = true;
        // 30 seconds for normal monsters, 180 seconds (3 mins) for boss
        monster.respawnTimer = monster.type === 'boss' ? 180 : 30;
      }
      syncHp = monster.hp;
    }
