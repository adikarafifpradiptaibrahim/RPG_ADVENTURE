...
export const HUD: React.FC<HUDProps> = ({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 select-none overflow-hidden font-sans">
      {/* 3D Floating Numbers / Combat Text */}
      <div className="absolute inset-0 pointer-events-none">
        {floatingTexts.map((ft) => (
          <div
            key={ft.id}
            className="absolute -translate-x-1/2 -translate-y-1/2 font-extrabold tracking-wider animate-bounce"
            style={{
              left: `${ft.x}%`,
              top: `${ft.y}%`,
              color: ft.color,
              textShadow: '0 2px 8px rgba(0,0,0,0.85), 0 0 12px rgba(0,0,0,0.5)',
              fontSize: ft.type === 'crit' ? '28px' : '20px',
            }}
          >
            {ft.text}
          </div>
      ...
      {nearbyPrompt && (
        <div className="pointer-events-auto absolute bottom-28 left-1/2 -translate-x-1/2 flex items-center justify-center">
          <button
      ...
      {/* Bottom Center: Combat Action Bar & Hotkeys */}
      <div className="pointer-events-auto absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 md:gap-2.5 bg-stone-950/85 backdrop-blur-md p-2 md:p-2.5 rounded-2xl border border-stone-800/80 shadow-2xl">
        {/* Hunter Weapon Switcher (Belati <-> Busur) */}
        {stats.characterClass === 'rogue' && onSwitchHunterWeapon && (
          <button
            onClick={onSwitchHunterWeapon}
            className="group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-emerald-950 to-stone-950 hover:from-emerald-900 hover:to-stone-900 text-emerald-300 rounded-xl border border-emerald-500/80 shadow cursor-pointer active:scale-95 transition-all"
            title={`Ganti Senjata Pemburu: ${hunterWeaponMode === 'bow' ? 'Beralih ke Belati' : 'Beralih ke Busur & Panah'} (X)`}
          >
            <span className="text-xl">{hunterWeaponMode === 'bow' ? '🏹' : '🗡️'}</span>
            <span className="text-[9px] font-bold text-emerald-200 line-clamp-1">
              {hunterWeaponMode === 'bow' ? 'Busur' : 'Belati'}
            </span>
            <span className="absolute -top-1.5 -right-1.5 bg-emerald-600 text-white text-[8px] font-mono px-1 rounded border border-emerald-400">
              X
            </span>
            <span className="absolute -bottom-1 text-[7px] text-emerald-400 font-mono font-bold bg-stone-900/95 px-0.5 rounded">
              Ganti
            </span>
          </button>
        )}
        ...
        <button
          ...
          className="group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-stone-800 to-stone-900 hover:from-stone-700 hover:to-stone-800 text-stone-100 rounded-xl border border-stone-600 shadow cursor-pointer active:scale-95 transition-all"
          title={`${classSkills.basic.label} (Klik Kiri / J)`}
        >
          <span className="text-xl">{classSkills.basic.icon}</span>
          <span className="text-[9px] font-bold text-stone-300 line-clamp-1">{classSkills.basic.label}</span>
          <span className="absolute -top-1.5 -right-1.5 bg-stone-700 text-stone-300 text-[8px] font-mono px-1 rounded border border-stone-500">
            J / Klik
          </span>
        </button>
        ...
        <button
          ...
          className={`group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-stone-900 to-stone-950 hover:brightness-110 rounded-xl border shadow cursor-pointer active:scale-95 transition-all ${classSkills.skill1.color}`}
          title={`${classSkills.skill1.name} (Q)`}
        >
          <span className="text-xl">{classSkills.skill1.icon}</span>
          <span className="text-[9px] font-bold line-clamp-1">{classSkills.skill1.name}</span>
          <span className="absolute -top-1.5 -right-1.5 bg-stone-800 text-white text-[8px] font-mono px-1 rounded border border-stone-600">
            {classSkills.skill1.key}
          </span>
          <span className="absolute -bottom-1 text-[7px] text-stone-400 font-mono font-bold bg-stone-900/90 px-0.5 rounded">
        ...
        <button
          ...
          className={`group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-stone-900 to-stone-950 hover:brightness-110 rounded-xl border shadow cursor-pointer active:scale-95 transition-all ${classSkills.skill2.color}`}
          title={`${classSkills.skill2.name} (E)`}
        >
          <span className="text-xl">{classSkills.skill2.icon}</span>
          <span className="text-[9px] font-bold line-clamp-1">{classSkills.skill2.name}</span>
          <span className="absolute -top-1.5 -right-1.5 bg-stone-800 text-white text-[8px] font-mono px-1 rounded border border-stone-600">
        ...
        <button
          ...
          className={`group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-stone-900 to-stone-950 hover:brightness-110 rounded-xl border shadow cursor-pointer active:scale-95 transition-all ${classSkills.skill3.color}`}
          title={`${classSkills.skill3.name} (R)`}
        >
          <span className="text-xl">{classSkills.skill3.icon}</span>
          <span className="text-[9px] font-bold line-clamp-1">{classSkills.skill3.name}</span>
          <span className="absolute -top-1.5 -right-1.5 bg-stone-800 text-white text-[8px] font-mono px-1 rounded border border-stone-600">
        ...
        <button
          onClick={onJump}
          className="group relative flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-b from-stone-800 to-stone-900 hover:from-stone-700 text-stone-200 rounded-xl border border-stone-600 shadow cursor-pointer active:scale-95 transition-all"
          title="Lompat / Menghindar (Spasi)"
        >
          <span className="text-xl">🦘</span>
          <span className="text-[9px] font-bold text-stone-400">Lompat</span>
          <span className="absolute -top-1.5 -right-1.5 bg-stone-700 text-stone-300 text-[8px] font-mono px-1 rounded border border-stone-500">
            Spasi
          </span>
        </button>
      ...
      {/* Bottom Right: Menu Navigation Buttons */}
      <div className="pointer-events-auto absolute bottom-5 right-4 flex items-center gap-2">
        <button
